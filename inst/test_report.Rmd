...
params:
  ...
  REF_TIME_PATH: "/Users/lily-clough/Documents/QCIRMS/220318_1pct_CO2_NaCl_NaSO4_brines/qc"
  RESULT_PATH: "/Users/lily-clough/Documents/data_processing_irms/OW_Schlenk_IsoData/OW_Schlenk_IsoData/results"
...
#library(QCIRMS)
source("../R/QAQC_IRMS_functions.r")
library(isoreader)
library(dplyr)
library(knitr)
```
...
#PLOTS_PATH <- RESULT_PATH # TODO: get rid of PLOTS_PATH

# get number of dxf directories 
dxf_dirs <- list.dirs(DATA_PATH,recursive=FALSE)
dxf_dirs <- dxf_dirs[!grepl("/\\.",dxf_dirs)] # remove hidden files from dxf_dirs
print(paste("Found ",as.character(length(dxf_dirs)), " directories of data",sep=""))
# loop over number of directories
for(i in seq(1,length(dxf_dirs))){
  ...
  WRITE_PATH <- paste(RESULT_PATH,"/",RESULTS_NAME,sep="")#params$RESULT_PATH
  REF_FILE <- params$REF_FILE
  CHECK_INTERNAL_STANDARDS <- params$CHECK_INTERNAL_STANDARDS
  
  path_tbl <- data.frame(
  Setting = c('DATA_PATH', 'REF_TIME_PATH', 'RESULT_PATH', 'WRITE_PATH', 'REF_FILE', 'CHECK_INTERNAL_STANDARDS'),
  Value = c(DATA_PATH, REF_TIME_PATH, RESULT_PATH, WRITE_PATH, REF_FILE, CHECK_INTERNAL_STANDARDS),
  stringsAsFactors = FALSE
  )
  knitr::kable(path_tbl, caption = 'Report parameters')

  
  # find dxf files in current directory 
  ...
  write.csv(expRef.df, file.path(WRITE_PATH, paste(RESULTS_NAME,'_expected_reference_times.csv',sep="")), row.names = FALSE)
 ...
 # process internal standards output if analyzed
 if(CHECK_INTERNAL_STANDARDS && length(currFiltered) >= 3){
  ...
  if(length(int_stand.list) >= 2 && !is.null(int_stand.list[[2]])){
    ...
    #write.csv(int_stand.list[[2]], file.path(WRITE_PATH, paste(RESULTS_NAME,'_internal_standard_means.csv',sep="")), row.names = FALSE)
    }
  ...
  if(length(int_stand.list) >= 3 && !is.null(int_stand.list[[3]])){
    ...
    #write.csv(int_stand.list[[3]], file.path(WRITE_PATH, paste(RESULTS_NAME,'_internal_standard_sds.csv',sep="")), row.names = FALSE)
    }
  }else{
  ...
  #write.csv(refs.dat, file.path(WRITE_PATH, paste(RESULTS_NAME,'_report_refs_passed.csv',sep="")), row.names = FALSE)
  #write.csv(samps.dat, file.path(WRITE_PATH, paste(RESULTS_NAME,'_report_samples_passed.csv',sep="")), row.names = FALSE)
  ...
  write.csv(summary_tbl, file.path(WRITE_PATH, paste(RESULTS_NAME,'_report_summary.csv',sep="")), row.names = FALSE)
  }
 ...
 if(!is.null(passFiles)){
  ...
  # passed QC
  generic_plot_all_raw(raw.list=passed_raw.list, path=WRITE_PATH,
                     write_pdf=T,
                     pdf_name=paste(RESULTS_NAME,"_passed_qc_spectra.pdf",sep="")) 
  # failed QC
  generic_plot_all_raw(failed_raw.list, path=WRITE_PATH,
                     write_pdf=T,
                     pdf_name=paste(RESULTS_NAME,"_failed_qc_spectra.pdf",sep="")) 
  } else{
