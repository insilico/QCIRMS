...
dxf_dirs <- dxf_dirs[!grepl("/\\.",dxf_dirs)] # remove hidden files from dxf_dirs vec
...
for(i in seq(1,length(dxf_dirs))){
  ...
  write.csv(expRef.df, file.path(WRITE_PATH, 'expected_reference_times.csv'), row.names = FALSE)
 ...
 if(CHECK_INTERNAL_STANDARDS && length(currFiltered) >= 3){
  ...
  if(length(int_stand.list) >= 2 && !is.null(int_stand.list[[2]])){
    ...
    write.csv(int_stand.list[[2]], file.path(WRITE_PATH, 'internal_standard_means.csv'), row.names = FALSE)
    }
  ...
  if(length(int_stand.list) >= 3 && !is.null(int_stand.list[[3]])){
    ...
    write.csv(int_stand.list[[3]], file.path(WRITE_PATH, 'internal_standard_sds.csv'), row.names = FALSE)
    }
  }else{
  ...
  write.csv(refs.dat, file.path(WRITE_PATH, 'report_refs_passed.csv'), row.names = FALSE)
  write.csv(samps.dat, file.path(WRITE_PATH, 'report_samples_passed.csv'), row.names = FALSE)
  ...
  write.csv(summary_tbl, file.path(WRITE_PATH, 'report_summary.csv'), row.names = FALSE)
  }
